...
namespace AdventOfCode
{
    class Day8 : PuzzleSolver
    {
        ...
        public Day8(string inputString) : base("Day Eight", inputString)
        {
        ...
        {
            SetupLayers();
            string image = FindImageWithFewestZeroDigits();
            MultiplyOneDigitsWithTwoDigits(image);
        }

        private void PuzzleTwo()
        {
            SetupLayers();
            string image = DecodeImage();
            PrintImagePassword(image);
        }

        private void SetupLayers()
        {
            SetupLayerSizeAndLayersCount();
            this.layers = new List<string>(this.layersCount);

            for (int i = 0; i < this.input.Length; i = i + this.layerSize)
            {
                this.layers.Add(this.input.Substring(i, this.layerSize));
            }
        }

        private void SetupLayerSizeAndLayersCount()
        {
            this.layerSize = 25 * 6;
            this.layersCount = this.input.Length / layerSize;
        }

        private string FindImageWithFewestZeroDigits()
        {
            int fewestZeroDigits = int.MaxValue;
            string imageWithFewestZeroDigits = "";

            foreach (var image in this.layers)
            {
                int zeroDigits = 0;
                ...
                if (zeroDigits < fewestZeroDigits)
                {
                    ...
                    imageWithFewestZeroDigits = image;
                }
            }

            return imageWithFewestZeroDigits;
        }

        private void MultiplyOneDigitsWithTwoDigits(string image)
        {
            int oneDigit = 0;
            int twoDigit = 0;
            for (int i = 0; i < image.Length; ++i)
            ...
            Console.WriteLine($"Puzzle one answer: {oneDigit * twoDigit}");
        }

        private string DecodeImage()
        {
            string image = "";
            bool foundBlackOrWhiteColor;
            for (int i = 0; i < this.layerSize; ++i)
            {
                foundBlackOrWhiteColor = false;
                for (int j = 0; j < this.layersCount; ++j)
                {
                    if (this.layers[j][i] == '0')
                    {
                        image += "0";
                        foundBlackOrWhiteColor = true;
                        break;
                    }
                    else if (this.layers[j][i] == '1')
                    {
                        image += "1";
                        foundBlackOrWhiteColor = true;
                        break;
        ...
        private void PrintImagePassword(string image)
        {
            for (int i = 1; i < 151; ++i)
            {
                if (image[i - 1] == '0')
...
-------------- Day 8: Space Image Format --------------
        ...
        You land your ship near the rover. When you reach the rover, you discover that it's already in the process of rebooting! 
        ...
        the second digit corresponds to the pixel to the right of that on the same layer, and so on until the last digit, which corresponds to the bottom-right pixel of the last layer.
